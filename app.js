...
function getQuizForModule(mid) {
  return quizData[mid] || defaultQuiz;
}
      ...
      <div class="quiz-list">
        ${modules.map(m => {
          ...
          return `<article class="quiz-module-card" data-start-quiz="${m.id}">
            <div class="emoji">${m.emoji}</div>
            <h3>${m.title}</h3>
            <p>${quizData[m.id] ? quizData[m.id].questions.length : defaultQuiz.questions.length} soal</p>
...
function showScore() {
  const qData = getQuizForModule(quizModuleId);
  const correct = qData.questions.reduce((t, q, i) => t + (quizAnswers[i] === q.answer ? 1 : 0), 0);
  const score = Math.round((correct / qData.questions.length) * 100);
  const passed = score >= 70;

  progress.quizScores[quizModuleId] = score;
  progress.quizPassed = quizPassedCount();
  saveProgress();
  routeTo('progress');
  showQuizProgressModal(score, correct, qData.questions.length, passed);
}
